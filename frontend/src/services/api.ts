...
export const categoriesApi = {
  ...
  async add(name: string): Promise<void> {
    return retryApiCall(async () => {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to add category (${response.status}): ${errorData.message || response.statusText}`);
      }
    }, 'Add category');
  },
  ...
  async delete(name: string): Promise<void> {
    return retryApiCall(async () => {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: response.statusText }));
        throw new Error(`Failed to delete category (${response.status}): ${errorData.message || response.statusText}`);
      }
    }, 'Delete category');
  },
  ...
  async rename(oldName: string, newName: string): Promise<void> {
    return retryApiCall(async () => {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: response.statusText }));
        throw new Error(`Failed to rename category (${response.status}): ${errorData.message || response.statusText}`);
      }
    }, 'Rename category');
  }
};
